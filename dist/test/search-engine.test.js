...
test("parseJavaSource extracts core symbols", () => {
    ...
    assert.ok(file.symbols.find((symbol) => symbol.kind === "extends" &&
        symbol.name === "BaseService" &&
        symbol.metadata?.subjectName === "OrderService"));
    assert.equal(file.symbols.filter((symbol) => symbol.kind === "implements").length, 2);
    assert.ok(file.symbols.find((symbol) => symbol.kind === "annotation_usage" &&
        symbol.name === "Transactional" &&
        symbol.metadata?.subjectName === "findOrder" &&
        symbol.metadata?.subjectKind === "method"));
    assert.equal(file.symbols.filter((symbol) => symbol.kind === "call").length, 2);
});
...
`
    ...
    assert.equal(extendsResolution.candidates[0]?.qualifiedName, "com.acme.demo.BaseService");
    ...
    assert.equal(annotationResolution.candidates[0]?.qualifiedName, "com.acme.demo.Transactional");
    const typeDefinitions = engine.findTypeDefinitions("BaseService", "/tmp/src/com/acme/demo/OrderService.java");
...
`
    ...
    assert.equal(findByIdResolution.candidates[0]?.qualifiedName, "com.acme.demo.Repository.findById");
    ...
    assert.equal(loadResolution.candidates[0]?.qualifiedName, "com.acme.demo.Repository.load");
});
...
test("JavaCodeSearchService persists snapshot and refreshes incrementally", async () => {
    ...
    try {
        ...
        await writeFile(repositoryPath, `
package com.acme.demo;
...
`, "utf8");
        await writeFile(orderServicePath, `
package com.acme.demo;
...
`, "utf8");
        ...
        assert.equal(reopened.search({ kind: "call", text: "load", exact: true }).length, 1);
        await writeFile(repositoryPath, `
package com.acme.demo;
...
`, "utf8");
        ...
        assert.equal(reopened.search({ kind: "method", text: "save", exact: true }).length, 1);
        await rm(orderServicePath);
        const fourthRefresh = await reopened.refresh();
        assert.equal(fourthRefresh.deleted.length, 1);
        assert.equal(reopened.search({ kind: "class", text: "OrderService", exact: true }).length, 0);
    }
    finally {
...
`, "utf8");
        ...
        assert.equal(service.search({ kind: "method", text: "save", exact: true }).length, 1);
    }
    finally {
        handle?.close();
        await rm(tempRoot, { recursive: true, force: true });
